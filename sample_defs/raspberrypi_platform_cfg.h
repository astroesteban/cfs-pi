...
**
**  \par Description:
**      The #CFE_PLATFORM_ES_RAM_DISK_MOUNT_STRING parameter is used to set the
*cFE mount path
**      for the CFE RAM disk. This is a parameter for missions that do not want
*to
**      use the default value of "/ram", or for missions that need to have a
*different
**      value for different CPUs or Spacecraft.
**      Note that the vxWorks OSAL cannot currently handle names that have more
*than one
**      path separator in it. The names "/ram", "/ramdisk", "/disk123" will all
*work, but
**      "/disks/ram" will not.
...
**
**  \par Description:
**       Dictates the maximum number of unique MsgIds the SB routing table will
*hold.
**       This constant has a direct effect on the size of SB's tables and
*arrays.
**       Keeping this count as low as possible will save memory.
**       To see the run-time, high-water mark and the current utilization
*figures
**       regarding this parameter, send an SB command to 'Send Statistics Pkt'.
**
**  \par Limits
**       This must be a power of two if software bus message routing hash
*implementation
**       is being used.  Lower than 64 will cause unit test failures, and
**       telemetry reporting is impacted below 32.  There is no hard
**       upper limit, but impacts memory footprint.  For software bus message
*routing
**       search implementation the number of msg ids subscribed to impacts
*performance.
**
*/
...
**
**  \par Description:
**       Dictates the maximum number of unique Pipes the SB routing table will
*hold.
**       This constant has a direct effect on the size of SB's tables and
*arrays.
**       Keeping this count as low as possible will save memory.
...
**
**  \par Limits
**       This parameter has a lower limit of 1.  This parameter must also be
*less than
...
/**
**  \cfesbcfg Maximum Number of unique local destinations a single MsgId can
*have
**
**  \par Description:
**       Dictates the maximum number of unique local destinations a single MsgId
*can
**       have.
**
**  \par Limits
**       This parameter has a lower limit of 1.  There are no restrictions on
*the upper
**       limit however, the maximum number of destinations per packet is system
*dependent
**       and should be verified.  Destination number values that are checked
*against this
...
**
**  \par Description:
**       Dictates the default Message Limit when using the #CFE_SB_Subscribe
*API. This will
**       limit the number of messages with a specific message ID that can be
*received through
**       a subscription. This only changes the default; other message limits can
*be set on a per
...
**       sends, the SB dynamically allocates from this memory pool, the memory
*needed
**       to process the message. The memory needed to process each message is
*msg
...
**
**  \par Limits
**       This parameter has a lower limit of 512 and an upper limit of UINT_MAX
*(4 Gigabytes).
**
*/
...
**
**  \par Description:
**       The value of this constant dictates the range of valid message ID's,
*from 0
**       to CFE_PLATFORM_SB_HIGHEST_VALID_MSGID (inclusive).
**
**       Although this can be defined differently across platforms, each
*platform can
**       only publish/subscribe to message ids within their allowable range.
*Typically
**       this value is set the same across all mission platforms to avoid this
*complexity.
**
**  \par Limits
**       CFE_SB_INVALID_MSG is set to the maximum representable number of type
*CFE_SB_MsgId_t.
**       CFE_PLATFORM_SB_HIGHEST_VALID_MSGID lower limit is 1, up to
*CFE_SB_INVALID_MSG_ID - 1.
**
**       When using the direct message map implementation for software bus
*routing, this
**       value is used to size the map where a value of 0x1FFF results in a 16
*KBytes map
**       and 0xFFFF is 128 KBytes.
**
**       When using the hash implementation for software bus routing, a multiple
*of the
**       CFE_PLATFORM_SB_MAX_MSG_IDS is used to size the message map.  In that
*case
**       the range selected here does not impact message map memory use, so it's
**       reasonable to use up to the full range supported by the message ID
*implementation.
*/
...
*software
**       bus routing information.  This filename is used only when no filename
*is
...
**       The value of this constant defines the filename used to store the
*software
**       bus message map information. This filename is used only when no
*filename is
**       specified in the command. The message map is a lookup table (array of
*16bit
**       words) that has an element for each possible MsgId value and holds the
**       routing table index for that MsgId. The Msg Map provides fast access to
*the
...
**       filtered through EVS. The filtering will begin after the SB task
*initializes
**       and stay in effect until a cmd to EVS changes it.
**       This allows the operator to set limits on the number of event messages
*that
...
**
**  \par Description:
**       This configuration parameter selects whether the Time task functions as
*a
**       time "server" or "client".  A time server generates the "time at the
*tone"
**       packet which is received by time clients.
**
**  \par Limits
**       Enable one, and only one by defining either
*CFE_PLATFORM_TIME_CFG_SERVER or
**       CFE_PLATFORM_TIME_CFG_CLIENT AS true.  The other must be defined as
*false.
*/
...
**
**  \par Description:
**       Depending on the specific hardware system configuration, it may be
*possible
**       for Time Servers to read the "local" MET from a h/w register rather
*than
**       having to track the MET as the count of tone signal interrupts (virtual
*MET)
**
**       Time Clients must be defined as using a virtual MET.  Also, a Time
*Server
**       cannot be defined as having both a h/w MET and an external time source
*(they
**       both cannot synchronize to the same tone).
**
**       Note: "disable" this define (set to false) only for Time Servers with
*local hardware
...
**       active tone signal. Both Time Clients and Time Servers support this
*feature.
**       Note: Set the CFE_PLATFORM_TIME_CFG_SIGNAL define to true to enable
*tone signal commands.
**
**  \par Limits
**       Not Applicable
*/
...
/**
**  \cfetimecfg Include or Exclude the Internal/External Time Source Selection
*Cmd
**
**  \par Description:
**       By default, Time Servers maintain time using an internal MET which may
*be a
...
**       internal MET, or external time data received from one of several
*supported
**       external time sources. Only a Time Server may be configured to use
*external
**       time data.
**       Note: Set the CFE_PLATFORM_TIME_CFG_SOURCE define to true to include
*the Time Source
**             Selection Command (command allows selection between the internal
**             or external time source). Then choose the external source with
*the
...
**
**  \par Description:
**       If #CFE_PLATFORM_TIME_CFG_SOURCE is set to true, then one of the
*following external time
**       source types must also be set to true.  Do not set any of the external
*time
**       source types to true unless #CFE_PLATFORM_TIME_CFG_SOURCE is set to
*true.
**
**  \par Limits
**       -# If #CFE_PLATFORM_TIME_CFG_SOURCE is set to true then one and only
*one of the following
**       three external time sources can and must be set true:
**       #CFE_PLATFORM_TIME_CFG_SRC_MET, #CFE_PLATFORM_TIME_CFG_SRC_GPS,
...
/**
**  \cfetimecfg Define the Max Delta Limits for Time Servers using an Ext Time
*Source
**
**  \par Description:
**       If #CFE_PLATFORM_TIME_CFG_SOURCE is set to true and one of the external
*time sources is
**       also set to true, then the delta time limits for range checking is
*used.
**
...
**       following defined amount, then the new time data will be ignored. This
*range
...
**
**  \par Limits
**       Applies only if both #CFE_PLATFORM_TIME_CFG_SERVER and
*#CFE_PLATFORM_TIME_CFG_SOURCE are set
**       to true.
*/
...
**
**  \par Description:
**       Specifies the capability of the local clock.  Indicates the time at
*which
...
**
**  \par Description:
**       Defines limits to the timing of the 1Hz tone signal. A tone signal is
*valid
**       only if it arrives within one second (plus or minus the tone limit)
*from
**       the previous tone signal.Units are microseconds as measured with the
*local
**       clock.
**
**  \par Limits
...
**
**  \par Description:
**       Define time to enter flywheel mode (in seconds since last tone data
*update)
...
**
**  \par Description:
**       Define Periodic Time to Update Local Clock Tone Latch. Applies only
*when
**       in flywheel mode. This define dictates the period at which the
*simulated
...
**
**  \par Limits
**       There is a lower limit of 6.  The lower limit corresponds to the cFE
*internal
**       applications.  There are no restrictions on the upper limit however,
*the
**       maximum number of applications is system dependent and should be
*verified.
**       AppIDs that are checked against this configuration are defined by a 32
*bit
**       data word.
*/
...
**
**  \par Description:
**       Defines the maximum number of cFE Shared libraries that can be loaded
*into
**       the system.
**
**  \par Limits
**       There is a lower limit of 1.  There are no restrictions on the upper
*limit
**       however, the maximum number of libraries is system dependent and should
*be
**       verified.
*/
...
**       There is a lower limit of 1.  There are no restrictions on the upper
*limit
**       however, the maximum number of log entries is system dependent and
*should be
**       verified.
*/
...
**
**  \par Limits
**       There is a lower limit of 512.  There are no restrictions on the upper
*limit
...
**
**  \par Description:
**       Defines the number of entries in the ES Object table. This table
*controls
**       the core cFE startup.
**
**  \par Limits
**       There is a lower limit of 15.  There are no restrictions on the upper
*limit
**       however, the maximum object table size is system dependent and should
*be
**       verified.
*/
...
**
**  \par Description:
**       ES Application Control Scan Rate. This parameter controls the speed
*that ES
**       scans the Application Table looking for App Delete/Restart/Reload
*requests.
**       All Applications are deleted, restarted, or reloaded by the ES
*Application.
**       ES will periodically scan for control requests to process. The scan
*rate is
**       controlled by this parameter, which is given in milliseconds. A value
*of
**       1000 means that ES will scan the Application Table once per second. Be
**       careful not to set the value of this too low, because ES will use more
*CPU
...
**      "scan periods" that ES will wait for an application to Exit after
*getting
**      the signal Delete, Reload or Restart. The sequence works as follows:
**      -# ES will set the control request for an App to Delete/Restart/Reload
*and
**         set this kill timer to the value in this parameter.
**      -# If the App is responding and Calls it's RunLoop function, it will
*drop out
**         of it's main loop and call CFE_ES_ExitApp. Once it calls Exit App,
*then
**         ES can delete, restart, or reload the app the next time it scans the
*app
**         table.
**      -# If the App is not responding, the ES App will decrement this Kill
*Timeout
**         value each time it runs. If the timeout value reaches zero, ES will
*kill
**         the app.
**
**      The Kill timeout value depends on the #CFE_PLATFORM_ES_APP_SCAN_RATE. If
*the Scan Rate
**      is 1000, or 1 second, and this #CFE_PLATFORM_ES_APP_KILL_TIMEOUT is set
*to 5, then it
**      will take 5 seconds to kill a non-responding App.
**      If the Scan Rate is 250, or 1/4 second, and the
*#CFE_PLATFORM_ES_APP_KILL_TIMEOUT is
**      set to 2, then it will take 1/2 second to time out.
**
**  \par Limits
**       There is a lower limit of 1 and an upper limit of 100 on this
*configuration
...
**
**  \par Description:
**       Defines the ram disk sector size. The ram disk is 1 of 4 memory areas
*that
...
**
**  \par Limits
**       There is a lower limit of 128.  There are no restrictions on the upper
*limit
**       however, the maximum RAM disk sector size is system dependent and
*should be
**       verified.
*/
...
**
**  \par Description:
**       Defines the ram disk number of sectors. The ram disk is one of four
*memory
...
**       There is a lower limit of 128.  There are no restrictions on the upper
*limit
**       however, the maximum number of RAM sectors is system dependent and
*should be
**       verified.
*/
...
**
**  \par Description:
**      The #CFE_PLATFORM_ES_RAM_DISK_PERCENT_RESERVED parameter is used to make
*sure that the
**      Volatile ( RAM ) Disk has a defined amount of free space during a
*processor
**      reset. The cFE uses the Volatile disk to decompress cFE applications
*during
**      system startup. If this Volatile disk happens to get filled with logs
*and
**      misc files, then a processor reset may not work, because there will be
*no
**      room to decompress cFE apps. To solve that problem, this parameter sets
*the
**      "Low Water Mark" for disk space on a Processor reset. It should be set
*to
**      allow the largest cFE Application to be decompressed.
**      During a Processor reset, if there is not sufficient space left on the
*disk,
...
**
**  \par Limits
**       There is a lower limit of 0 and an upper limit of 75 on this
*configuration
**       parameter.Units are percentage. A setting of zero will turn this
*feature
**       off.
*/
...
**
**  \par Description:
**       Defines the Critical Data Store (CDS) area size in bytes size. The CDS
*is
...
**
**  \par Limits
**       There is a lower limit of 8192 and an upper limit of UINT_MAX (4
*Gigabytes)
...
**       reserved Memory area. This is a block of memory that is available for
*cFE
**       application use. The address is obtained by calling
**       #CFE_PSP_GetUserReservedArea. The User Reserved Memory is one of four
*memory
...
**
**  \par Limits
**       There is a lower limit of 1024 and an upper limit of UINT_MAX (4
*Gigabytes)
**       on this configuration parameter.
*/
...
*variable
**       and log area. This is a block of memory used by the cFE to store the
*system
**       log ER Log and critical reset variables. This is 4 of 4 of the memory
*areas
**       that are preserved during a processor reset.
**       Note: This area must be sized large enough to hold all of the data
**       structures. It should be automatically sized based on the
*CFE_ES_ResetData_t
**       type, but circular dependencies in the headers prevent it from being
*defined
**       this way.
**       NOTE: Changing this value changes memory allocation, and may
**       require changes to platform specific values (in CFE_PSP) such as
...
**
**  \par Limits
**       There is a lower limit of 153600 (150KBytes) and an upper limit of
*UINT_MAX
...
**
**  \par Limits
**       This must always be a power of 2, as it is used as a binary address
*mask.
*/
...
**       contains a list of modules that will be loaded and started by the cFE
*after
...
**
**  \par Description:
**       The value of this constant defines the filename used to store
*information
**       pertaining to all of the Applications that are registered with
*Executive
**       Services. This filename is used only when no filename is specified in
*the
...
**       information (as ASCII text strings) that might not be able to be sent
*in an
**       Event Message. This filename is used only when no filename is specified
*in
**       the command to dump the system log. No file specified in the cmd means
*the
...
**  \par Description:
**       The value of this constant defines the filename used to store the
**       Exception and Reset (ER) Log. This filename is used only when no
*filename is
**       specified in the command to dump the ER log. No file specified in the
*cmd
**       means the first character in the cmd filename is a NULL terminator
*(zero).
**
**  \par Limits
**       The length of each string, including the NULL terminator cannot exceed
...
**  \par Description:
**       The value of this constant defines the filename used to store the
**       Performance Data. This filename is used only when no filename is
*specified
...
**  \par Description:
**       The value of this constant defines the filename used to store the
**       Critical Data Store Registry. This filename is used only when no
*filename is
...
**
**  \par Description:
**       Defines the default mode for the operation of the ES System log
*following a power
**       on reset. The log may operate in either Overwrite mode = 0, where once
*the
**       log becomes full the oldest message in the log will be overwritten, or
**       Discard mode = 1, where once the log becomes full the contents of the
*log are
**       preserved and the new event is discarded.  This constant may hold a
*value of
**       either 0 or 1 depending on the desired default.
**       Overwrite Mode = 0, Discard Mode = 1.
**
**  \par Limits
**       There is a lower limit of 0 and an upper limit of 1 on this
*configuration
**       parameter.
*/
...
**       Defines the default mode for the operation of the ES System log
*following a
**       processor reset. The log may operate in either Overwrite mode = 0,
*where once
**       the log becomes full the oldest message in the log will be overwritten,
*or
**       Discard mode = 1, where once the log becomes full the contents of the
*log are
**       preserved and the new event is discarded.  This constant may hold a
...
**
**  \par Description:
**       Defines the maximum size of the performance data buffer. Units are
*number of
**       performance data entries. An entry is defined by a 32 bit data word
*followed
**       by a 64 bit time stamp.
**
**  \par Limits
**       There is a lower limit of 1025.  There are no restrictions on the upper
*limit
**       however, the maximum buffer size is system dependent and should be
*verified.
**       The units are number of entries. An entry is defined by a 32 bit data
*word followed
**       by a 64 bit time stamp.
*/
...
**
**  \par Description:
**       Defines the filter mask for disabling all performance entries. The
*value is a
**       bit mask.  For each bit, 0 means the corresponding entry is disabled
*and
...
**
**  \par Description:
**       Defines the filter mask for enabling all performance entries. The value
*is a
**       bit mask.  For each bit, 0 means the corresponding entry is disabled
*and
**       1 means it is enabled.
...
**
**  \par Description:
**       Defines the default filter mask for the performance data buffer. The
*value is a
**       bit mask.  For each bit, 0 means the corresponding entry is disabled
*and 1
**       means it is enabled.
...
**
**  \par Description:
**       Defines the default trigger mask for disabling all performance data
*entries. The value
**       is a bit mask.  For each bit, 0 means the trigger for the corresponding
*entry is
...
**
**  \par Description:
**       Defines the trigger mask for enabling all performance data entries. The
*value is
**       a bit mask.  For each bit, 0 means the trigger for the corresponding
*entry is
**       disabled and 1 means it is enabled.
**
*/
...
**
**  \par Description:
**       Defines the default trigger mask for the performance data buffer. The
*value is a
**       32-bit mask.  For each bit, 0 means the trigger for the corresponding
*entry is
**       disabled and 1 means it is enabled.
**
*/
...
**
**  \par Limits
**       It is recommended this parameter be greater than or equal to 4KB. This
*parameter
**       is limited by the maximum value allowed by the data type. In this case,
*the data
**       type is an unsigned 32-bit integer, so the valid range is 0 to
*0xFFFFFFFF.
*/
...
**
**  \par Description:
**       This parameter defines the delay time (in milliseconds) between
*performance
**       data file writes performed by the Executive Services Performance
*Analyzer
**       Child Task.
**
**  \par Limits
**       It is recommended this parameter be greater than or equal to 20ms. This
*parameter
**       is limited by the maximum value allowed by the data type. In this case,
*the data
**       type is an unsigned 32-bit integer, so the valid range is 0 to
...
/**
**  \cfeescfg Define Performance Analyzer Child Task Number of Entries Between
*Delay
**
**  \par Description:
**       This parameter defines the number of performance analyzer entries the
*Performance
...
**
**  \par Description:
**       This parameter defines a default stack size. This parameter is used by
*the
**       cFE Core Applications.
**
**  \par Limits
**       There is a lower limit of 2048.  There are no restrictions on the upper
*limit
**       however, the maximum stack size is system dependent and should be
*verified.
**       Most operating systems provide tools for measuring the amount of stack
*used by a
**       task during operation. It is always a good idea to verify that no more
*than 1/2
...
**       is system dependent and should be verified.  Most operating systems
*provide
**       tools for measuring the amount of stack used by a task during
*operation. It
**       is always a good idea to verify that no more than 1/2 of the stack is
*used.
*/
#define CFE_PLATFORM_EVS_START_TASK_STACK_SIZE \
...
*used.
*/
#define CFE_PLATFORM_TIME_START_TASK_STACK_SIZE \
  CFE_PLATFORM_ES_DEFAULT_STACK_SIZE
...
*used.
*/
#define CFE_PLATFORM_TBL_START_TASK_STACK_SIZE \
  CFE_PLATFORM_ES_DEFAULT_STACK_SIZE
...
**
**  \par Limits
**       There is a lower limit of 8.  There are no restrictions on the upper
*limit
...
**
**  \par Description:
**       Number of Processor Resets before a Power On Reset is called. If set to
*2,
**       then 2 processor resets will occur, and the 3rd processor reset will be
*a
**       power on reset instead.
**
**  \par Limits
**       There is a lower limit of 0.  There are no restrictions on the upper
*limit
**       however, the maximum number of processor resets may be system dependent
*and
**       should be verified.
*/
...
**       is using the CFE_ES Memory Pool APIs (#CFE_ES_PoolCreate,
*#CFE_ES_PoolCreateNoSem,
**       #CFE_ES_GetPoolBuf and #CFE_ES_PutPoolBuf) but finds these sizes
**       inappropriate for their use, they may wish to use the
*#CFE_ES_PoolCreateEx
...
**       These sizes MUST be increasing and MUST be an integral multiple of 4.
*Also,
**       CFE_PLATFORM_ES_MAX_BLOCK_SIZE must be larger than
*CFE_MISSION_SB_MAX_SB_MSG_SIZE and both
**       CFE_PLATFORM_TBL_MAX_SNGL_TABLE_SIZE and
*CFE_PLATFORM_TBL_MAX_DBL_TABLE_SIZE.  Note that if Table
**       Services have been removed from the CFE, the table size limits are
*still
...
**       Services local event log. This filename is used only when no filename
*is
...
**
**  \par Description:
**       Dictates the EVS local event log capacity. Units are the number of
*events.
**
**  \par Limits
**       There are no restrictions on the lower and upper limits however,
...
**       used only when no filename is specified in the command to dump the
*event
**       log.
**
**  \par Limits
...
**
**  \par Description:
**       Defines the default port state (enabled or disabled) for the four
*output
**       ports defined within the Event Service. Port 1 is usually the uart
*output
...
**       bit 1 = Info, bit 2 = Error, bit 3 = Critical). This is a global
*setting,
**       meaning it applies to all applications. To filter an event type, set
*its
...
**
**  \par Description:
**       Defines the default message format (long or short) for event messages
*being
**       sent to the ground. Choose between #CFE_EVS_MsgFormat_LONG or
**       #CFE_EVS_MsgFormat_SHORT.
**
**  \par Limits
**       The valid settings are #CFE_EVS_MsgFormat_LONG or
*#CFE_EVS_MsgFormat_SHORT
*/
...
**
**  \par Description:
**       Defines the TOTAL size of the memory pool that cFE Table Services
*allocates
**       from the system.  The size must be large enough to provide memory for
*each
**       registered table, the inactive buffers for double buffered tables and
*for
...
**       Therefore, this size will be multiplied by
...
**       The cFE does not place a limit on the size of this parameter but it
*must be
**       small enough to allow for #CFE_PLATFORM_TBL_MAX_SIMULTANEOUS_LOADS
*number of tables
...
**
**  \par Description:
**       Defines the maximum number of tables supported by this processor's
*Table Services.
**
**  \par Limits
**       This number must be less than 32767.  It should be recognized that this
*parameter
**       determines the size of the Table Registry.  An excessively high number
*will waste
**       memory.
*/
...
**
**  \par Description:
**       Defines the maximum number of critical tables supported by this
*processor's Table Services.
**
**  \par Limits
**       This number must be less than 32767.  It should be recognized that this
*parameter
**       determines the size of the Critical Table Registry which is maintained
*in the Critical
**       Data Store.  An excessively high number will waste Critical Data Store
*memory.  Therefore,
**       this number must not exceed the value defined in
*CFE_ES_CDS_MAX_CRITICAL_TABLES.
*/
...
**
**  \par Limits
**       This number must be less than 32767.  This number must be at least as
*big as
**       the number of tables (#CFE_PLATFORM_TBL_MAX_NUM_TABLES) and should be
*set higher if tables
...
#define CFE_PLATFORM_TBL_U32FROM4CHARS(_C1, _C2, _C3, _C4)          \
  ((uint32)(_C1) << 24 | (uint32)(_C2) << 16 | (uint32)(_C3) << 8 | \
   (uint32)(_C4))
...
#define CFE_PLATFORM_TBL_VALID_SCID_2 \
...
#define CFE_PLATFORM_TBL_VALID_PRID_2 \
  (CFE_PLATFORM_TBL_U32FROM4CHARS('a', 'b', 'c', 'd'))
...
**
**  \par Description:
**      During startup, some tasks may need to synchronize their own
*initialization
**      with the initialization of other applications in the system.
**
**      CFE ES implements an API to accomplish this, that performs a task delay
*(sleep)
**      while polling the overall system state until other tasks are ready.
**
**      This value controls the amount of time that the
*CFE_ES_ApplicationSyncDelay
**      will sleep between each check of the system state.  This should be large
*enough
**      to allow other tasks to run, but not so large as to noticeably delay the
*startup
**      completion.
**
...
**      "ready" states.
**
**      The CFE "main" thread starts individual tasks for each of the core
*applications
**      (except FS).  Each of these must perform some initialization work before
*the
**      next core application can be started, so the main thread waits to ensure
*that the
**      application has reached the "ready" state before starting the next
*application.
**
**      If any core application fails to start, then it indicates a major
*problem with
...
**
**  \par Description:
**      The upper limit for the total amount of time that all apps listed in the
*CFE ES startup
**      script may take to all become ready.
**
**      Unlike the "core" app timeout, this is a soft limit; if the allotted
*time is exceeded,
**      it probably indicates an issue with one of the apps, but does not cause
*CFE ES to take
